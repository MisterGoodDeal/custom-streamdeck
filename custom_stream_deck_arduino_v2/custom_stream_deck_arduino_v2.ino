...
boolean isKeyboardFunction = false;

int LED = 2; // Pin for the LED mode
int blinkDelay = 150; // Delay in ms
...
void loop() {
  ...
  if (customKey) {
    if (isKeyboardFunction) {
      ...
      Keyboard.write(tolower(customKey));
    }
  }
  if (isKeyboardFunction) {
...
void keypadEvent(KeypadEvent customKey) {
  switch (customKeypad.getState()) {
    case HOLD:
      if (customKey == 'T') {
        isKeyboardFunction = !isKeyboardFunction;
        Serial.print("SWITCHING_FUNCTION => ");
        Serial.println(isKeyboardFunction);
        blinkLED();
      }
      break;
  }
